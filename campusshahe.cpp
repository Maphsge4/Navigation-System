...
campusShahe::campusShahe(QWidget *parent) : QMainWindow(parent)
{
    ...
    header->setFont(font);
    QString str = QString("北邮沙河校区导航");
    header->setText(str);
    header->setFixedSize(300, 60);
...
/* ------------------------------------------------------------------------------------------------------------------ */
    ...
    connect(searchBtn, &QPushButton::clicked, [=](){
        ...
        int curMode = 0; // 沙河 沙河
        if (chooseStartCampus->currentText() == "沙河" && chooseEndCampus->currentText() == "本部")
            curMode = 1; // 沙河 本部
        else
            ;
        ...
        if (srcLine->text() != "" && destLine->text() != "") // 有输入再执行
        {
            if (curMode == 0) // 沙河 沙河
            {
                if (shaheCampus->castShahe.contains(srcLine->text()) && shaheCampus->castShahe.contains(destLine->text()))
                {
                    if (srcLine->text() != destLine->text())
                    {
                        ...
                        {
                            this->navigationStrategy = MinDistance;
                            if (chooseStartCampus->currentText() == "沙河" && chooseEndCampus->currentText() == "沙河")
                            {
                                this->pathAll = shaheCampus->dijkstra(src, dest, MinDistance, SHAHECAMPUS);
                                if (destLine->text() == "S4")
                                    insideMode = DORMITORY;
                                else if (destLine->text() == "N教学楼")
                                    insideMode = CLASSROOM;
                            }
                        }
                        // 方案：途径某点的最短距离
                        else if (choosePlan->currentText() == "途经最短")
                        {
                            this->navigationStrategy = MinDistance;
                            // 起点->途径点
                            this->pathAll = shaheCampus->dijkstra(src, pass, MinDistance, SHAHECAMPUS);
                            // 途经点->终点
                            QVector<path> ans = shaheCampus->dijkstra(pass, dest, MinDistance, SHAHECAMPUS);
                            // 起点->途径点->终点
                        ...
                        else if (choosePlan->currentText() == "最短时间")
                        {
                            ...
                            this->pathAll = shaheCampus->dijkstra(src, dest, MinTime, SHAHECAMPUS);
                        }
                        else if (choosePlan->currentText() == "交通工具")
                        {
                            this->navigationStrategy = MinBike;
                            this->pathAll = shaheCampus->dijkstra(src, dest, MinBike, SHAHECAMPUS);
                        }
                ...
                else if (shaheCampus->castShahe.contains(srcLine->text()) && destLine->text() == "食堂")
                {
                    QString src = srcLine->text();
                    this->navigationStrategy = MinDistance;
                    showDetailedInfoKind = CANTEEN;
                    detailedInfo = new showDetailedInfo;
                    detailedInfo->show();
                    connect(detailedInfo, &showDetailedInfo::detailedInfoBack, [=](){
                        if (chooseCanteen == 1)
                        {
                            destLine->setText("学生食堂1楼");
                            this->pathAll = shaheCampus->dijkstra(src, "学生食堂", MinDistance, SHAHECAMPUS);
                        }
                        else if (chooseCanteen == 2)
                        {
                            destLine->setText("学生食堂2楼");
                            this->pathAll = shaheCampus->dijkstra(src, "学生食堂", MinDistance, SHAHECAMPUS);
                        }
                        else if (chooseCanteen == 3)
                        {
                            destLine->setText("学生食堂3楼");
                            this->pathAll = shaheCampus->dijkstra(src, "学生食堂", MinDistance, SHAHECAMPUS);
                        }
                        else if (chooseCanteen == 4)
                        {
                            destLine->setText("学生食堂4楼");
                            this->pathAll = shaheCampus->dijkstra(src, "学生食堂", MinDistance, SHAHECAMPUS);
                        }
                        else if (chooseCanteen == 5)
                        {
                            destLine->setText("学生食堂5楼");
                            this->pathAll = shaheCampus->dijkstra(src, "学生食堂", MinDistance, SHAHECAMPUS);
                        }
                        else if (chooseCanteen == 6)
                        {
                            destLine->setText("教工食堂1楼");
                            this->pathAll = shaheCampus->dijkstra(src, "教工食堂", MinDistance, SHAHECAMPUS);
                        }
                        else if (chooseCanteen == 7)
                        {
                            destLine->setText("教工食堂2楼");
                            this->pathAll = shaheCampus->dijkstra(src, "教工食堂", MinDistance, SHAHECAMPUS);
                        }
                        else if (chooseCanteen == 8)
                        {
                            destLine->setText("教工食堂3楼");
                            this->pathAll = shaheCampus->dijkstra(src, "教工食堂", MinDistance, SHAHECAMPUS);
                        }
                        else if (chooseCanteen == 9)
                        {
                            destLine->setText("教工食堂4楼");
                            this->pathAll = shaheCampus->dijkstra(src, "教工食堂", MinDistance, SHAHECAMPUS);
                        }
                        else if (chooseCanteen == 10)
                        {
                            destLine->setText("教工食堂5楼");
                            this->pathAll = shaheCampus->dijkstra(src, "教工食堂", MinDistance, SHAHECAMPUS);
                        }
                        // 更新人当前点的信息
                        this->curPosition.pathId = 0;
            ...
            else if (curMode == 1) // 沙河 本部
            {
                if (shaheCampus->castShahe.contains(srcLine->text()) && shaheCampus->castBenbu.contains(destLine->text()))
                {
                    this->changeFlag = 0;
                    QString src = srcLine->text();
                    QString dest = destLine->text();
                    ...
                    connect(detailedInfo, &showDetailedInfo::detailedInfoBack, [=](){
                        if (chooseTransportMethod == 1)
                        {
                            ...
                            if (choosePlan->currentText() == "最短距离")
                            {
                                ...
                                chooseEndCampus->setCurrentText("沙河");
                                this->pathAll = shaheCampus->dijkstra(src, "校车点", MinDistance, SHAHECAMPUS);
                                this->navigationStrategy = MinDistance;
                                finalName = dest;
                                this->changeFlag = 1;
                            }
                        ...
                        else if (chooseTransportMethod == 2)
                        {
                            ...
                            if (choosePlan->currentText() == "最短距离")
                            {
                                destLine->setText("西门");
                                chooseEndCampus->setCurrentText("沙河");
                                this->pathAll = shaheCampus->dijkstra(src, "西门", MinDistance, SHAHECAMPUS);
                                this->navigationStrategy = MinDistance;
                                finalName = dest;
                                this->changeFlag = 1;
                            }
    ...
    if (campusTransferMode == BenbuToShahe)
    {
        ...
        if (chooseTransportMethod == 1)
        {
            ...
            srcLine->setText("校车点");
            this->pathAll = shaheCampus->dijkstra("校车点", finalName, MinDistance, SHAHECAMPUS);
        }
        else if (chooseTransportMethod == 2)
        {
            QTextStream textStream(&myFile);
            ...
            srcLine->setText("西门");
            this->pathAll = shaheCampus->dijkstra("西门", finalName, MinDistance, SHAHECAMPUS);
        }
    ...
    connect(timer, &QTimer::timeout, [=](){
        ...
        // 如果要换场景，去本部（并且是校车）
        if (this->curPosition.x == 986 && this->curPosition.y == 524 && this->changeFlag && chooseTransportMethod == 1)
        {
            QDateTime current_date_time = QDateTime::currentDateTime();
            QString current_date = current_date_time.toString("yyyy.MM.dd ddd hh:mm:ss");
            textStream << current_date;
            ...
            this->changeFlag = 0;
            campusTransferMode = ShaheToBenbu;
        ...
        // 如果要换场景，去本部（并且是公交车）
        if (this->curPosition.x == 80 && this->curPosition.y == 524 && this->changeFlag && chooseTransportMethod == 2)
        {
            QDateTime current_date_time = QDateTime::currentDateTime();
            QString current_date = current_date_time.toString("yyyy.MM.dd ddd hh:mm:ss");
            textStream << current_date;
        ...
        // 要去教室，N教学楼
        if (this->curPosition.x == 1159 && this->curPosition.y == 580 && insideMode == CLASSROOM)
        {
            this->insideMap = new inside;
            QTimer::singleShot(500, this, [=](){
                this->insideMap->setGeometry(this->geometry());
                this->insideMap->show();
                this->hide();
            });
        }
        ...
        // 要去宿舍，S4宿舍楼
        if (this->curPosition.x == 614 && this->curPosition.y == 781 && insideMode == DORMITORY)
        {
            this->insideMap = new inside;
        ...
        shaheCampus->castShahe["当前位置"] = 75;
        shaheCampus->p_shahe[75] = tmpPoint;
        // 初始化“当前位置”点到其他所有点的距离为inf
        for (int i = 0; i < 100; i ++)
        {
            shaheCampus->e_shahe[75][i].dis = inf;
            shaheCampus->e_shahe[75][i].timeDis = inf;
            shaheCampus->e_shahe[75][i].bikeDis = inf;
        }
        for (int i = 0; i < 100; i ++)
        {
            shaheCampus->e_shahe[i][75].dis = inf;
            shaheCampus->e_shahe[i][75].timeDis = inf;
            shaheCampus->e_shahe[i][75].bikeDis = inf;
        }
        // 到自己为 0
        shaheCampus->e_shahe[75][75].dis = 0;
        shaheCampus->e_shahe[75][75].timeDis = 0;
        shaheCampus->e_shahe[75][75].bikeDis = 0;
        ...
        if (this->curPosition.pathId != this->pathAll.size()) // 没越界
        {
            ...
            shaheCampus->e_shahe[75][startId].tongxing = 1;//通行情况
            shaheCampus->e_shahe[75][startId].dir = this->pathAll[this->curPosition.pathId].flag;//水平竖直
            if(shaheCampus->e_shahe[75][startId].dir == 1) //水平
                shaheCampus->e_shahe[75][startId].dis = shaheCampus->p_shahe[75].x - shaheCampus->p_shahe[startId].x;//距离
            else // 垂直
                shaheCampus->e_shahe[75][startId].dis = shaheCampus->p_shahe[75].y - shaheCampus->p_shahe[startId].y;//距离
            shaheCampus->e_shahe[75][startId].crowd = shaheCampus->e_shahe[startId][endId].crowd;
            shaheCampus->e_shahe[75][startId].bicycle = shaheCampus->e_shahe[startId][endId].bicycle;//自行车


            shaheCampus->e_shahe[75][endId].tongxing = 1;//通行情况
            shaheCampus->e_shahe[75][endId].dir = this->pathAll[this->curPosition.pathId].flag;//水平竖直
            if(shaheCampus->e_shahe[75][endId].dir == 1) //水平
                shaheCampus->e_shahe[75][endId].dis = shaheCampus->p_shahe[75].x - shaheCampus->p_shahe[endId].x;//距离
            else
                shaheCampus->e_shahe[75][endId].dis = shaheCampus->p_shahe[75].y - shaheCampus->p_shahe[endId].y;//距离
            shaheCampus->e_shahe[75][endId].crowd = shaheCampus->e_shahe[startId][endId].crowd;
            shaheCampus->e_shahe[75][endId].bicycle = shaheCampus->e_shahe[startId][endId].bicycle;//自行车
        }
        else // 如果已经到了最后一条边，就不能访问pathId了，需要-1
        {
            ...
            shaheCampus->e_shahe[75][startId].tongxing = 1;//通行情况
            shaheCampus->e_shahe[75][startId].dir = this->pathAll[this->curPosition.pathId - 1].flag;//水平竖直
            if(shaheCampus->e_shahe[75][startId].dir == 1) //水平
                shaheCampus->e_shahe[75][startId].dis = shaheCampus->p_shahe[75].x - shaheCampus->p_shahe[startId].x;//距离
            else
                shaheCampus->e_shahe[75][startId].dis = shaheCampus->p_shahe[75].y - shaheCampus->p_shahe[startId].y;//距离
            ...
            shaheCampus->e_shahe[75][endId].tongxing = 1;//通行情况
            shaheCampus->e_shahe[75][endId].dir = this->pathAll[this->curPosition.pathId - 1].flag;//水平竖直
            if(shaheCampus->e_shahe[75][endId].dir == 1) //水平
                shaheCampus->e_shahe[75][endId].dis = shaheCampus->p_shahe[75].x - shaheCampus->p_shahe[endId].x;//距离
            else
                shaheCampus->e_shahe[75][endId].dis = shaheCampus->p_shahe[75].y - shaheCampus->p_shahe[endId].y;//距离
        ...
        // 边可能是负数，改为正
        if(shaheCampus->e_shahe[75][startId].dis < 0)
            shaheCampus->e_shahe[75][startId].dis = -shaheCampus->e_shahe[75][startId].dis;
        if(shaheCampus->e_shahe[75][endId].dis < 0)
            shaheCampus->e_shahe[75][endId].dis = -shaheCampus->e_shahe[75][endId].dis;
        // 无向图设置
        shaheCampus->e_shahe[startId][75].dis = shaheCampus->e_shahe[75][startId].dis;
        shaheCampus->e_shahe[endId][75].dis = shaheCampus->e_shahe[75][endId].dis;

        // 设置 timeDis
        shaheCampus->e_shahe[startId][75].timeDis = (int)(shaheCampus->e_shahe[startId][75].dis / shaheCampus->e_shahe[startId][75].crowd);
        shaheCampus->e_shahe[75][startId].timeDis = shaheCampus->e_shahe[startId][75].timeDis;

        shaheCampus->e_shahe[endId][75].timeDis = (int)(shaheCampus->e_shahe[endId][75].dis / shaheCampus->e_shahe[endId][75].crowd);
        shaheCampus->e_shahe[75][endId].timeDis = shaheCampus->e_shahe[endId][75].timeDis;

        // 设置 bikeDis
        if (shaheCampus->e_shahe[75][endId].bicycle == true)
            shaheCampus->e_shahe[75][endId].bikeDis = shaheCampus->e_shahe[75][endId].dis;
        else
            shaheCampus->e_shahe[75][endId].bikeDis = inf;
        shaheCampus->e_shahe[endId][75].bikeDis = shaheCampus->e_shahe[75][endId].bikeDis;

        if (shaheCampus->e_shahe[75][startId].bicycle == true)
            shaheCampus->e_shahe[75][startId].bikeDis = shaheCampus->e_shahe[75][startId].dis;
        else
            shaheCampus->e_shahe[75][startId].bikeDis = inf;
        shaheCampus->e_shahe[startId][75].bikeDis = shaheCampus->e_shahe[75][startId].bikeDis;


         // 获取周围最近的5个点的信息，放入this->pointsAround
        this->pointsAround = shaheCampus->findPointsAround(tmpPoint, SHAHECAMPUS);
...
//            item->setTextColor(QColor(200,111,100));
    ...
    connect(showAroundPath_1, &QPushButton::clicked, [=](){
        if (showAroundPath_1->text() == "具体路径")
        {
            ...
            this->curToAroundPath = shaheCampus->dijkstra("当前位置", this->pointsAround[0].name, MinDistance, SHAHECAMPUS);
            // 停止原来的路径
            timer->stop();
    ...
    connect(showAroundPath_2, &QPushButton::clicked, [=](){
        ...
        {
            this->curToAroundPath = shaheCampus->dijkstra("当前位置", this->pointsAround[1].name, MinDistance, SHAHECAMPUS);
            timer->stop();
    ...
    connect(showAroundPath_3, &QPushButton::clicked, [=](){
        ...
        {
            this->curToAroundPath = shaheCampus->dijkstra("当前位置", this->pointsAround[2].name, MinDistance, SHAHECAMPUS);
            timer->stop();
            // 如果到终点了，就不能变为继续，否则又回起动计时器
            if(this->curPosition.x == this->pathAll[this->pathAll.size() - 1].end.x && this->curPosition.y == this->pathAll[this->pathAll.size() - 1].end.y)
                ;
    ...
    connect(showAroundPath_4, &QPushButton::clicked, [=](){
        ...
        {
            this->curToAroundPath = shaheCampus->dijkstra("当前位置", this->pointsAround[3].name, MinDistance, SHAHECAMPUS);
            timer->stop();
            // 如果到终点了，就不能变为继续，否则又回起动计时器
            if(this->curPosition.x == this->pathAll[this->pathAll.size() - 1].end.x && this->curPosition.y == this->pathAll[this->pathAll.size() - 1].end.y)
                ;
    ...
    connect(showAroundPath_5, &QPushButton::clicked, [=](){
        ...
        {
            this->curToAroundPath = shaheCampus->dijkstra("当前位置", this->pointsAround[4].name, MinDistance, SHAHECAMPUS);
            timer->stop();
            // 如果到终点了，就不能变为继续，否则又回起动计时器
            if(this->curPosition.x == this->pathAll[this->pathAll.size() - 1].end.x && this->curPosition.y == this->pathAll[this->pathAll.size() - 1].end.y)
                ;
    ...
    connect(changeBtn, &QPushButton::clicked, [=](){
        timer->stop();
        // 以前这里还加入“当前位置”这个点，现在觉得没必要，因为上面每次人坐标更新都会添加 “当前位置”这个点
        srcLine->setText("当前位置");
    });
}

void campusShahe::paintEvent(QPaintEvent *)
{
    // 设置画家
    ...
    {
        if (this->shaheCampus != NULL)
        {
            for (int i = 1; i <= 74; i++)
                for (int j = 1; j <= 74; j++)
                {
                    if (this->shaheCampus->e_shahe[i][j].bicycle == false)
                    {
                        // 当前路径所在边的起点 （*0.6是因为图片缩小了0.6倍，+28是偏移）
                        painter.drawEllipse(QPoint(this->shaheCampus->p_shahe[i].x * 0.6 + 28, this->shaheCampus->p_shahe[i].y * 0.6 + 62), 5, 5);
                        // 当前路径所在边的终点
                        painter.drawEllipse(QPoint(this->shaheCampus->p_shahe[j].x * 0.6 + 28, this->shaheCampus->p_shahe[j].y * 0.6 + 62), 5, 5);
                        // 当前路径所在边 起点到终点的连线
                        painter.drawLine(QPoint(this->shaheCampus->p_shahe[i].x * 0.6 + 28, this->shaheCampus->p_shahe[i].y * 0.6 + 62), QPoint(this->shaheCampus->p_shahe[j].x * 0.6 + 28, this->shaheCampus->p_shahe[j].y * 0.6 + 62));
                    }
                }
        }
    }
    ...
    if (this->navigationStrategy == MinTime)
    {
        for (int i = 0; i < this->pathAll.size(); i ++)
        {
            ...
            if (this->pathAll[i].crowd >= 0.5)
            {
                ...
                painter.setBrush(brush);
                // 当前路径所在边的起点 （*0.6是因为图片缩小了0.6倍，+28是偏移）
                painter.drawEllipse(QPoint(this->pathAll[i].start.x * 0.6 + 28, this->pathAll[i].start.y * 0.6 + 62), 5, 5);
                // 当前路径所在边的终点
                painter.drawEllipse(QPoint(this->pathAll[i].end.x * 0.6 + 28, this->pathAll[i].end.y * 0.6 + 62), 5, 5);
                // 当前路径所在边 起点到终点的连线
                painter.drawLine(QPoint(this->pathAll[i].start.x * 0.6 + 28, this->pathAll[i].start.y * 0.6 + 62), QPoint(this->pathAll[i].end.x * 0.6 + 28, this->pathAll[i].end.y * 0.6 + 62));
            }
    ...
    painter.drawEllipse(QPoint(this->curPosition.x * 0.6 + 28, this->curPosition.y * 0.6 + 62), 6, 6);
    ...
    {
        // 起点
        painter.drawEllipse(QPoint(this->curToAroundPath[i].start.x * 0.6 + 28, this->curToAroundPath[i].start.y * 0.6 + 62), 4, 4);
        // 终点
        painter.drawEllipse(QPoint(this->curToAroundPath[i].end.x * 0.6 + 28, this->curToAroundPath[i].end.y * 0.6 + 62), 4, 4);
        // 起点到终点的连线
        painter.drawLine(QPoint(this->curToAroundPath[i].start.x * 0.6 + 28, this->curToAroundPath[i].start.y * 0.6 + 62), QPoint(this->curToAroundPath[i].end.x * 0.6 + 28, this->curToAroundPath[i].end.y * 0.6 + 62));
    }
}


QString campusShahe::showNextSchoolBus(QString presentTime)
{
...
QString campusShahe::showNextBus(QString presentTime)
{
    char *ch;
    QByteArray ba= presentTime.toLatin1();
    ch = ba.data();
